...
T_O_A =  ([cos(Alpha) -sin(Alpha) 0 PositionObstacle(1);
              sin(Alpha) cos(Alpha) 0 PositionObstacle(2);
              0 0 1 0;
              0 0 0 1]);
          
CoordonneeRepereObstacle = inv(T_O_A) * [Xreel; Yreel; 0; 1];
...
if (X_Prime <= 0) %Alors Attraction vers le cycle limite 
    RayonCycleLimite=GetRv(Obstacle(IndiceObstaclePlusProche))-5;
else %Période de l'extraction du cycle limite 
    RayonCycleLimite=RayonCycleLimite+5; %Pour que le changement de rayon soit en douceur. 
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Y_dot = sign(X_Prime) * Xrelatif + (Mu * Yrelatif * ((RayonCycleLimite^2) - (Xrelatif^2) - (Yrelatif^2)));
