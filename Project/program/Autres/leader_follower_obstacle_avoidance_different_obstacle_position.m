...
initial_positions = [1 1.3 1.6 1.3; -2 -3.5 -2 -1.5; 0.4, 0.1, 0.1, 0.4];
    ...
    for i = 2:N
        ...
        for j = neighbors
            ...
            for m = 1:length(obstacles)
                s(m) = norm(x(1:2, i) - obstacles(:, m));
            end

            minn = min(s);

            if (minn > 0.5)
                dxi(:, i) = dxi(:, i) + formation_control_gain * (norm(x(1:2, j) - x(1:2, i)) ^ 2 - desired_distance ^ 2) * 4.1 * (x(1:2, j) - x(1:2, i));
