...
for t = 2:iterations
    [x, expected_odometer, odometer, th_part, r_isol, r_global, sigmax, sigmay] = localization(t, r, expected_odometer, N, odometer, podo, Q, waypoints, tfault1, tfault2, tfault3, tfault4, R, sigmax, sigmay);
...
hold on;
plot(-3 * sigmax(2:iterations), 'r', 'LineWidth', 1.5);
title('Errors in X_odo with 99.7% Confidence Interval');
legend('3*sigma', 'X_odo Errors', '-3*sigma');
grid on;

% Subplot for Y_odo errors
subplot(2, 1, 2);
...
hold on;
plot(-3 * sigmay(2:iterations), 'r', 'LineWidth', 1.5);
title('Errors in Y_odo with 99.7% Confidence Interval');
legend('3*sigma', 'Y_odo Errors', '-3*sigma');
grid on;

% Adjust layout for better appearance
sgtitle('Combined Errors in X_odo and Y_odo');

% figure(4) % Erreurs en X_odo et la région critique à 99.7%
...
% plot(-3*sigmax(2:iterations), 'r')
%
% figure(5) % Erreurs en Y_odo et la région critique à 99.7%
...
% ylim([0, communication_timeout]); % Set y_odo-axis limit for better readability
...
function marker_size = determine_marker_size(robotarium_instance, marker_size_meters)
    ...
    % normalized so you could do this with y_odo and figure height as well).
...
function font_size = determine_font_size(robotarium_instance, font_height_meters)
    ...
    % Determine the ratio of the font height to the y_odo-axis
