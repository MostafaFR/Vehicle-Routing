...
for t = 2:iterations
    ...
    if length(next_indices) < 4
        next_indices = [next_indices, 1:(4 - length(next_indices))];
    end
    ...
    if distance_to_waypoint < close_enough
        ...
        savedPoints = curvePoints(end - 2 * numPoints:end - 2 * numPoints - 1, :);
        control_points(1, :) = x(1:2, 1)';
        control_points(5, :) = waypoints(:, next_indices(3))';
        control_points(6, :) = control_points(1, :);
        curvePoints = calculateCatmullRomCurve(control_points, numPoints); % Recalcul de la courbe
        % curvePoints = curvePoints(waypoint_index -10:end, :); % Conserve uniquement les points non parcourus
        set(curve_line, 'XData', curvePoints(:, 1), 'YData', curvePoints(:, 2)); % Mise à jour de la courbe
    end
