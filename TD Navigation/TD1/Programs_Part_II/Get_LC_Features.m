...
function [LCCost ID_CollidedObstcale RobotTrajectory DataCostDetails DataGain] = ...
    ...
    AngleEntreAxesRepereObstacle = GetAngleEntreAxesRepere(Obstacle(ID_Obstacle)); % % % R�cup�ration des caract�ristiques du rep�re li� � l'obstacle � �viter
    CentreRepereEvitement = GetCentreRepere(Obstacle(ID_Obstacle)); % %C'est le centre du rep�re li� � l'obstacle
    PointsTangentes = GetPointsTangentes(Obstacle(ID_Obstacle)); % %Get Points from the Final Target

    %Initialisation de la structure robot [x, y, theta, Gamma] d�di�e � cette fonction
    ...
    RobotTrajectory = [];
    NombreIterationsMax = 6000; % %C'est le nombre d'it�rations max que peut avoir le cycle-limite � obtenir
    inc = 0;
    LinearVelocity = 0;
    ...
    for i = 1:NombreIterationsMax
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%Obtention de la consigne [ModuleVitesse AngleVitesse] � injecter au niveau de la commande
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        %%D�termination de la position relative du robot par rapport � l'obstacle
        Xrelatif = Xreel - X0;
        Yrelatif = Yreel - Y0;

        RayonCycleLimite = A + Marge;

        %%Pour d�finir le sens de rotation "clockwise" ou "counter-clockwise"
        if (bClockwise == 1)
            Y_Prime = 1;
        else
            Y_Prime = -1;
        ...
        ModuleVitesse = 0.2; % % Vitesse d'�volution constante pour la planification
        AngleVitesse = atan2(Y_dot, X_dot);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

        %%Application du contr�leur angulaire simple proportionnel
        K = 1; %gain par rapport � la commande angulaire
        ...
        LinearVelocity = PasEchantillonnageCommande * ModuleVitesse; % %Cette ligne est l� juste pour la sauvegarde des consignes des vitesses, utilisable lors de la restitution de la courbe de vitesse
        ...
        if (((SignePostionRobot_X_OLD <= 0) && (SignePostionRobot_X > 0)) && (inc > 1)) % %Pour dire qu'on switch uniquement quand on passe du signe moins au plus et non une autre combinaison
            RayonCible = 0.1; % %C'est le rayon � partir duquel on consid�re que la cible est atteinte
            D = sqrt(Position(1) ^ 2 + Position(2) ^ 2); % %On calcul ainsi cette distance car Position est donn�e par rapport au centre du rep�re de l'obstacle
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%Affichage ou pas du r�sultat de la solution interm�diaire locale (par rapport � un seul obstacle) optimale si elle existe !!
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
